...
export default function Header() {
  ...
  useEffect(() => {
    ...
    if (isSessionDropdownOpen || isBlogDropdownOpen) {
          ...
          <div className="hidden md:flex items-center space-x-6 text-stone-900 text-lg tracking-wider font-primary">
            ...
            >
              ...
              {isSessionDropdownOpen && (
                <div className="absolute left-0 top-full w-auto py-2 z-50">
                  <div className='bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden'>
                  <Link href="#reviews" className="block px-4 py-2 text-stone-900 font-primary hover:bg-[rgb(245,124,0)] hover:text-white transition-colors duration-200" onClick={() => setIsSessionDropdownOpen(false)}>reviews</Link>
                  <Link href="#faq" className="block px-4 py-2 text-stone-900 font-primary hover:bg-[rgb(245,124,0)] hover:text-white transition-colors duration-200" onClick={() => setIsSessionDropdownOpen(false)}>FAQ</Link>
                  </div>
                </div>
              )}
            </div>
            ...
            >
              <div className="flex items-center">
                <Link
                  ...
                  tabIndex={0}
                  onClick={() => setIsBlogDropdownOpen((open) => !open)}
                >
                  blog
                  <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                </Link>
              ...
              {isBlogDropdownOpen && (
                <div className="absolute left-0 top-full w-auto py-2 z-50">
                  <div className='bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden'>
                    <Link href="#newsletter" className="block px-4 py-2 text-stone-900 font-primary hover:bg-[rgb(245,124,0)] hover:text-white transition-colors duration-200" onClick={() => setIsBlogDropdownOpen(false)}>newsletter</Link>
                  </div>
                </div>
              )}
            </div>
            ...
            <nav className="flex flex-col space-y-3">
              ...
              <div className="relative">
                <button
                  ...
                  className={`text-black transition-colors duration-200 px-4 py-2 rounded-full font-primary ${getHoverColor()} flex items-center justify-between w-full`}
                >
                  blog
                ...
                {isBlogDropdownOpen && (
                  <div className="mt-2 ml-4">
                    <Link
                      href="#newsletter"
                      className="block text-black transition-colors duration-200 px-4 py-2 rounded-full font-primary hover:bg-[rgb(245,124,0)] hover:text-white"
                      onClick={() => {
                        setIsMenuOpen(false)
                        setIsBlogDropdownOpen(false)
                      }}
