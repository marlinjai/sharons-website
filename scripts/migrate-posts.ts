...
// Migration script to import existing blog posts from BlogData.tsx to SQLite database
// Run with: npx ts-node --compiler-options '{"module":"commonjs"}' scripts/migrate-posts.ts

import { getDb, createPost, getAllPosts, generateSlug } from '../lib/db';

// Existing blog posts data (copied from blogPosts/BlogData.tsx)
// Note: We extract just the text content, stripping React components
const existingPosts = [
  {
    id: 2,
    ...
    date: 'July 15, 2025',
    image:
      'https://images.unsplash.com/photo-1597673814716-4a1e58a1f6af?q=80&w=2940&auto=format&fit=crop',
    published: false,
...
<p>Ask questions. Follow your gut. And remember: real transformation doesn't always follow a linear path, but it should always feel aligned.</p>`,
  },
  {
    ...
    date: 'June 15, 2025',
    image:
      'https://images.unsplash.com/photo-1512641406448-6574e777bec6?q=80&w=3087&auto=format&fit=crop',
    published: false,
...
async function migrate() {
  ...
  if (existing.length > 0) {
    console.log(`Database already has ${existing.length} posts. Skipping migration.`);
    console.log('To re-run migration, delete the data/blog.db file first.');
    return;
  }
  ...
  console.log(`\nMigration complete! Imported ${existingPosts.length} posts.`);
  console.log('Posts are set to unpublished by default. Use the admin panel to publish them.');
}
